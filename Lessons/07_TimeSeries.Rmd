...
```{r}
#the first thing you do with time series analysis is plot the variable over time 
...
```{r}
#can also get column by doing dataset[[columnnumber]]
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365)
#if you want to see the time series, you can type in the console head(timeseriesname, num er of observations you want to see)
```
