...
# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) #double == means set to this year

# Choose multiple conditions to filter
summary(NTL.phys.data$lakename) #uniquejust returns unique values (names in this case)
NTL.phys.data.PeterPaul <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))

#%in% command goes through every row and looks for specific values in following vector and includes only those 
...
#find a range of days using %in% day 152-304
...
NTL.phys.data.1990.1999.tuesday<-filter(NTL.phys.data.1990.1999, lakename=="Tuesday Lake")

#How to pipe these? 
NTL.phys.data.90.99.tuesday2<-
  NTL.phys.data %>%
...
John.way<-filter(NTL.phys.data, year4 == 1999 & lakename=="Tuesday Lake")
...
#create a new column, lubridate has a fxn called month??? 


#this is wrong but idk what it is
...
exercise5<-pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Irradiance")

#Exercise 6: spread temperatureC into more than one column based on the depth

exercise6<-pivot_wider(NTL.phys.data, temperature_C)
```
