...
```{r}
#lm(dataset$variable y ~dataset$variable x )
...
#meaningful regression, small p-value
...
>  significant negative correleation between irradiance and depth 
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #divides plot screen into four rows, mar defines limits of grids 
plot(irradiance.regression)
par(mfrow = c(1,1))
#plot 1- looking for symmetry and flat red line (residuals vs fitted)
#plot 2- want circles to be on dashed line (q-q)
#plot 3- looking for symmetry again 
#plot 4- helps identify outliers, if lay outside dashed lines 
```
...
#we can see relationship is not linear, so perhaps try transforming irrandiance (log)
```
...
#!=0 removes values=zero, and irradiancewater<5000 removes outlier
#notice that r-squared is much larger now than before (data is better explained by this model)
...
#scale_y_log10 makes y axis the log transformed data

# SE - confidence interval around smooth can also be removed (se=FALSE)
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point() +
...
#residuals wonky on first plot, line isn't straight
#how can we improve? look for more explanatory variables
...
#mixed means ellipses and numbers both in the plot, cool! 
```
...
#looking at total phosphorus as a function of all the other variables
