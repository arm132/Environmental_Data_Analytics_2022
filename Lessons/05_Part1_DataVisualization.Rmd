...
the things in the top of the r chunk determine size of plot, apply to every plot in the whole chunk (cool)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? color means contour and fill is the inside of the shape, doesn't look at good with boxplots
print(Nutrientplot3)
...
print(Nutrientplot4)
#position=dodge makes the points go out to the side instead of just sitting on top of one another, stack=center is making the dots go up from the center line 

# Violin plot, also is a density plot
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Frequency polygons
# Using a tidy dataset, missing some labels 
Nutrientplot6 <-
...
# Using a gathered dataset, this is going to make the plot better
Nutrientplot7 <-   
...
# Ridgeline plot, show density over time 
Nutrientplot6 <-
