...
#inner_join: if there is a month of rainfall where we don't have temp, r just assigns na but won't delete. use this in the assignment!! you can also use left_join 
#we don't like to use column bind here bc we aren't sure if they are matching up times
...
#the temperature seems to have an upward trend over time

rainfall_plot<- ggplot(BRA_complete, aes(x=Date, y=Rainfall)) +
  geom_line()+
  geom_smooth(method="lm")
print(rainfall_plot)
#rainfall might have an upward trend? but it's hard to see. there is definitely some seasonal variation 
...
#and then plug these in start=c(fyear,fmonth)
...
#can identify all components, seasonal, trend, and remainder 
...
temp.components<- as.data.frame(temp_decomposed$time.series[,1:3])

temp.components<- mutate(temp.components,
        Observed = BRA_complete$Temperature_C,     
        Date = BRA_complete$Date)

rainfall.components<- as.data.frame(rainfall_decomposed$time.series[,1:3])

rainfall.components<- mutate(rainfall.components,
        Observed = BRA_complete$Rainfall,     
        Date = BRA_complete$Date)

# Exercise 10: Visualize how the trend maps onto the data for both series

#ERROR: Error in FUN(X[[i]], ...) : object 'Observed' not found 
temp_plot2<-ggplot(BRA_complete) +
...
rainfall_plot2<- ggplot(BRA_complete, aes(x=Date, y=Rainfall)) +
  geom_line()+
  geom_smooth(method="lm")
print(rainfall_plot)
