...
#shapiro is just telling you if data is normal, bell shaped curve 
#histogram has a tail on the right (not quite normal) 
#Q-Q plot points do not follow the diagonal line super well (banana shape)
...
#doing a t-test
#hnull, sample is = to 50, ha, sample is less than 50 
#p-value is very low, reject null hypothesis
#sample suggests that this location has a mean below 50 ppm
...
> We found that ozone levels are below the 50 ppm thershold using a one-sided t-test (p<2.2e-16). Our sample failed the Shapiro-wilk test of normailty. 
...
h0: samples have same mean 
ha: samples have different means 
```{r}
shapiro.test(EPAair$Ozone[EPAair$Year == 2018])
shapiro.test(EPAair$Ozone[EPAair$Year == 2019])
#low p-values mean these are NOT NORMAL distributions
...
ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly()
#we keep seeing this weird dip in the middle of the hump, something strange is going on 

# Format as a t-test
#only have two categories here, if we had more than two we would do an anova
...
#1
ozone.by.year<-ggplot(EPAair, aes(x=Ozone, fill=as.factor(Year)))+
  geom_density(alpha=0.5)+
  labs(title="Density Plot: Ozone by Year", y="Density", x="Ozone AQI Value", fill="Year")+
  scale_fill_manual(values = c('blue', 'pink'))

print(ozone.by.year)

#2
ozone.by.year2<-ggplot(EPAair, aes(y=Ozone, x=as.factor(Year)))+
  geom_boxplot()+
  stat_summary(geom="text", fun.y=max, vjust=-1, size=4, label=c("b", "a"), color="red")+
  labs(x="", y="Ozone AQI Value")+
  ylim(0,150)

print(ozone.by.year2)
  
#3
ozone.by.year3<-ggplot(EPAair, aes(x=as.factor(Year),y=Ozone ))+
  geom_violin(draw_quantiles = 0.5)+
  stat_summary(geom="text", fun.y=max, vjust=-1, size=4, label=c("b", "a"), color="red")+
  labs(x="", y="Ozone AQI Value")

print(ozone.by.year3)
...
ozone.pm.regression<-lm(data=EPAair, PM2.5 ~ Ozone)
summary(ozone.pm.regression)

#r-squared=.1314 (not good)
...
pm25scatter<-ggplot(EPAair, aes(x=Ozone, y=PM2.5))+
  geom_point(alpha=.5)+
...
qqnorm(EPAair$PM2.5); qqline(EPAair$PM2.5) #lots of zeros
...
#reminder! select is choosing variables 
